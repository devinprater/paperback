...
const WXK_F7: i32 = 346;
const WXK_F7_GTK: i32 = 65_476;
const WXK_F10: i32 = 349;

#[cfg(target_os = "linux")]
fn linux_heading_level_key(ch: char) -> Option<i32> {
	match ch {
		'1' | '!' => Some(1),
		'2' | '@' => Some(2),
		'3' | '#' => Some(3),
...
#[cfg(target_os = "linux")]
fn linux_reader_navigation_command(
	...
	if let Some(level) = linux_heading_level_key(ch) {
...
impl DocumentManager {
	...
	fn build_text_ctrl(panel: Panel, word_wrap: bool, self_rc: &Rc<Mutex<Self>>) -> TextCtrl {
		...
		let dm_for_shortcuts = Rc::clone(self_rc);
		let text_ctrl_for_menu = text_ctrl;
		text_ctrl.on_key_down(move |event| {
			if let WindowEventData::Keyboard(kbd) = &event {
				#[cfg(target_os = "linux")]
				if let Some(command_id) = linux_reader_navigation_command(
					kbd.get_unicode_key(),
					kbd.get_key_code(),
					kbd.shift_down(),
					...
					if let Ok(dm) = dm_for_shortcuts.try_lock() {
						let frame = dm.frame;
						drop(dm);
						frame.post_menu_command(command_id);
					}
					kbd.event.skip(false);
					return;
				}
				if let Some(key) = kbd.get_key_code() {
					if (key == WXK_F7 || key == WXK_F7_GTK)
						&& !kbd.shift_down()
						&& !kbd.cmd_down() && !kbd.control_down()
						&& !kbd.alt_down() && !kbd.meta_down()
					{
						if let Ok(dm) = dm_for_shortcuts.try_lock() {
							let frame = dm.frame;
							drop(dm);
							frame.post_menu_command(menu_ids::ELEMENTS_LIST);
						}
						kbd.event.skip(false);
						return;
					}
